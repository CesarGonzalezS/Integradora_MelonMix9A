...
import lambdas.admin_management.read_admin.app as lambda_function  # Adjust import path as per your actual structure

class TestLambdaHandler(unittest.TestCase):

    @patch('mysql.connector.connect')
    @patch.dict(os.environ, {'RDS_HOST': 'test_host', 'RDS_USER': 'test_user', 'RDS_PASSWORD': 'test_password', 'RDS_DB': 'test_db'})
    def test_successful_admin_read(self, mock_connect):
        ...
        mock_cursor.fetchone.return_value = (1, 'test_user', 'test_email', 'test_password', 'test_role')
        ...
        expected_sql = "SELECT * FROM admin WHERE admin_id = %s"
        expected_values = (1,)
        mock_cursor.execute.assert_called_once_with(expected_sql, expected_values)

    @patch('mysql.connector.connect')
    @patch.dict(os.environ, {'RDS_HOST': 'test_host', 'RDS_USER': 'test_user', 'RDS_PASSWORD': 'test_password', 'RDS_DB': 'test_db'})
    def test_admin_not_found(self, mock_connect):
        # Setup mock for MySQL connection and cursor
        mock_connection = MagicMock()
        mock_cursor = mock_connection.cursor()
        mock_connect.return_value = mock_connection
        ...
        self.assertEqual(json.loads(response['body']), 'Admin not found')
