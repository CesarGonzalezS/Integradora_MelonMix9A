...
import lambdas.admin_management.delete_admin.app as lambda_function  # Adjust import path as per your actual structure

class TestLambdaHandler(unittest.TestCase):

    @patch('mysql.connector.connect')
    @patch.dict(os.environ, {'RDS_HOST': 'test_host', 'RDS_USER': 'test_user', 'RDS_PASSWORD': 'test_password', 'RDS_DB': 'test_db'})
    def test_successful_admin_deletion(self, mock_connect):
        ...
        self.assertEqual(json.loads(response['body']), 'Admin deleted successfully')

        expected_sql = "DELETE FROM admin WHERE admin_id = %s"
        expected_values = (1,)
